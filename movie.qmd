...
rt |>
  ...
  text_transform(
    ...
    fn = \(x) {
      map(suppressWarnings(as.numeric(x)), \(rating) {
        if (is.na(rating)) {
          return(html("Not Featured in Top 250"))
        }
        rounded_rating <- floor(rating + 0.5)
        label <- sprintf(
          "%s out of %s (Rounded to nearest integer)",
          rating,
          10
        )
  ...
  fmt_markdown(columns = c(name:consensus, imdb1)) |>
  cols_merge(
  ...
  tab_options(
    ...
    heading.border.bottom.color = "#20794d",
  ...
  opt_css(
    ...
    .gt_row {
      border-bottom-color: #20794d !important;
